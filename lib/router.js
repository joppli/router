...
validRole = (acl, roles) => roles.some((role) => acl.indexOf(role) >= 0);

module.exports = (bus, routes, debugMode = false) =>
{
  // logger for debugging
  const debug = new Debug({debug:debugMode});
  ...
  (function walker(collection, dispatchers, i = 0)
  {
    ...
    {
      debug.log(`triggered: "${collection[i].name}" dispatchers: "${dispatchers}"`);
      try
      {
        // optinal acl validation
        !collection[i].acl || (args[0] && validRole(collection[i].acl, args[0].role || []))
        ...
        ? dispatchers.forEach((dispatcher) =>
          {
            if(~dispatcher.indexOf('::'))
            {
              ...
              debug.log(`dispatching: "${file}" -> "${action}"`);
              require(file)[action](...args);
            }
            else
            {
              debug.log(`dispatching: "${dispatcher}"`);
              require(dispatcher)(...args);
            }
          })
        : debug.log(`access not granted to "${collection[i].name}", `
             +`role: "${args[0].role}" not in acl: "${collection[i].acl}"`);
      }
      catch(e)
      {
        debug.log(`dispatcher threw an exception`);
